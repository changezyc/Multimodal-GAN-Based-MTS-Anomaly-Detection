...
def test(test_loader_seq, generator_rnn, generator_cnn, discriminator, encoder_rnn, encoder_cnn):
    ...
    for batch, (inputs, labels) in enumerate(test_loader_seq):
        ...
        x_r = inputs
        x_r_c = torch.empty(batch_size, 37, 37).to(device)
        ...
        x_cos = torch.empty(batch_size, 37, 37).to(device)
        for i in range(len(inputs)):
    ...
    y_predict_rec = detect_anomaly(anomaly_score=anomaly_score_rec, epsilon1=1.5,  epsilon2=99, method=1)
    # y_predict_rec = prune_single_positive(is_anomaly=y_predict_rec)
    y_predict_dis = detect_anomaly(anomaly_score=anomaly_score_dis, epsilon1=0.05, epsilon2=99, method=2)
    ...
    y_predict_together = detect_anomaly(anomaly_score, epsilon1=-0.3,  epsilon2=-0.18, method=1)
