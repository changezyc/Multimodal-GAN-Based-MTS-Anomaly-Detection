...
def discriminator_iteration(inputs_c, alpha):
    ...
    fake_x_r_c = torch.empty(batch_size, 37, 37).to(device)
...
def generator_rnn_iteration():
    ...
    # z = torch.from_numpy(z).to(torch.float32).to(device)
    x_ = generator_rnn(z)
    new_x_ = torch.empty(batch_size, 37, 37).to(device)
    ...
    for epoch in range(n_epochs):
        ...
        for i in range(n_discriminators):
            ...
            for batch, (inputs_c, labels) in enumerate(train_loader_cos):
                ...
                loss_discriminator = discriminator_iteration(inputs_c, alpha=0.8).item()
...
if __name__ == "__main__":
    ...
    fPath_Normal_v0 = 'kddcup_normal.csv'
    fPath_Attack_v0 = 'kddcup_test.csv'
