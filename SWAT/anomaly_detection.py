...
def test(test_loader_seq, generator_rnn, generator_cnn, discriminator, encoder_rnn, encoder_cnn):
    ...
    for batch, (inputs, labels) in enumerate(test_loader_seq):
        ...
        x_r = inputs
        x_r_c = torch.empty(batch_size, 51, 51).to(device)
        ...
        x_cos = torch.empty(batch_size, 51, 51).to(device)
        for i in range(len(inputs)):
    ...
    anomaly_score_rec_pre = theta * anomaly_score_rec_time + (1 - theta) * anomaly_score_rec_space
    anomaly_score_dis_pre = np.array(dis_score)
    anomaly_score_rec = stats.zscore((anomaly_score_rec_pre))
    anomaly_score_dis = stats.zscore(anomaly_score_dis_pre)

    y_predict_rec = detect_anomaly(anomaly_score=anomaly_score_rec, epsilon1=1.0, epsilon2=99, method=2)
    y_predict_rec = prune_single_positive(is_anomaly=y_predict_rec)
    y_predict_dis = detect_anomaly(anomaly_score=anomaly_score_dis, epsilon1=0.54, epsilon2=1.43, method=2)
    y_predict_dis = prune_single_positive(is_anomaly=y_predict_dis)

    temp_anoscore = (1 - lambda_) * anomaly_score_rec_pre + lambda_ * anomaly_score_dis_pre
    temp_anoscore.tolist()
    anomaly_score.extend(temp_anoscore)
    anomaly_score = stats.zscore(np.array(anomaly_score))
    ...
    y_predict_together = detect_anomaly(anomaly_score, epsilon1=0.12, epsilon2=99, method=2)
