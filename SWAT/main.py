...
def discriminator_iteration(inputs_c, alpha):
    ...
    fake_x_r_c = torch.empty(batch_size, 51, 51).to(device)
...
def generator_rnn_iteration():
    ...
    # z = torch.from_numpy(z).to(torch.float32).to(device)
    x_ = generator_rnn(z)
    new_x_ = torch.empty(batch_size, 51, 51).to(device)
    ...
    for epoch in range(n_epochs):
        ...
        for i in range(n_discriminators):
            ...
            for batch, (inputs_c, labels) in enumerate(train_loader_cos):
                ...
                loss_discriminator = discriminator_iteration(inputs_c, alpha=0.2).item()
...
if __name__ == "__main__":
    ...
    dataset_train, label_train, dataset_test, label_test = utils.process_datacsv_to_normal_distribution(fPath_Normal_v0,
                                                                                                        fPath_Attack_v0)
